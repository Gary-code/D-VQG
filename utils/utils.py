...
def save_checkpoint(data_name, epoch, epochs_since_improvement, chg_det_state, speaker_state, optimizer,
                    bleu4, is_best, cfg):
    """
    ...
    state = {'epoch': epoch,
             ...
             'bleu-4': bleu4,
             'change_detector': chg_det_state,
             'speaker': speaker_state,
             'decoder_optimizer': optimizer}
    # filename = config.checkpoint_path + 'best_checkpoint_85_gt_w_1_loss_no_det.pth.tar'
    filename = 'checkpoint_{}_final_6clips.pth.tar'.format(cfg.dataset_name)

    # filename = config.checkpoint_path + 'best_checkpoint_msvd_qa_6clips.pth.tar'
    torch.save(state, filename)


def save_checkpoint_1(checkpoint, filename):
