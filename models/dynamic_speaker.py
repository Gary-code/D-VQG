...
from .VideoQGModel import VideoQGModel
...
class DynamicCore(nn.Module):
    def __init__(self, cfg):
        ...
        self.lang_lstm = nn.LSTMCell(cfg.model.speaker.word_embed_size + \
                                     cfg.model.speaker.input_dim + cfg.model.speaker.rnn_size,
                                     cfg.model.speaker.rnn_size)
        # self.lang_lstm = nn.LSTMCell(cfg.model.speaker.word_embed_size + \
    ...
    def forward(self, xt,
                ...
                feat_diff, state, question_categories):

        prev_h = state[0][-1]  # prev hidden state from the lang_lstm
        # prev_h = question_categories
        ...
        # print(xt.shape, att_feat.shape, question_categories.shape)
        lang_lstm_input = torch.cat([xt, att_feat, question_categories], 1)
...
class DynamicSpeaker(CaptionModel):
    def __init__(self, cfg):
        ...
        self.embed_dim = cfg.model.speaker.rnn_size
        self.motion_dim = cfg.model.speaker.motion_input_dim
        self.dataset_name = cfg.data.dataset

        self.question_type_embedding = nn.Embedding(self.vocab_size, self.embed_dim)
        self.question_category_embedding = nn.Embedding(self.vocab_size, self.embed_dim)
        self.motion_feature_fc = nn.Linear(self.motion_dim, self.embed_dim)
        self.qt_fc = weight_norm(nn.Linear(self.embed_dim * 2, self.embed_dim))
    ...
    def _forward(self,
                 feat_bef, feat_aft,
                 feat_diff, seq, question_types, question_categories, encoded_answers):
        question_restrain = self.question_type_embedding(question_types)[:, 0, :]
        question_categories = self.question_category_embedding(question_categories)[:, 0, :] if self.dataset_name == 'svqa_dataset' else self.motion_feature_fc(question_categories).max(dim=1)[0]
        answers_embedding = self.embedding(encoded_answers)
        answers_lstm, (h_a, c_a) = self.lstm(answers_embedding)
        h1 = self.qt_fc(torch.cat([h_a[0], question_restrain], dim=1))
        c1 = self.qt_fc(torch.cat([c_a[0], question_restrain], dim=1))
        # start fresh
        self.module_weights = []

        batch_size = feat_bef.size(0)
        state = self.init_hidden(batch_size)
        state[0][1], state[1][1] = h1, c1
    ...
    def get_logprobs_state(self, it,
        ...
        output, state = self.core(xt, feat_bef, feat_aft, feat_diff, state, question_categories)
    ...
    def _sample(self,
                feat_bef, feat_aft,
                feat_diff, seq, question_types, question_categories, encoded_answers, cfg={}, sample_max=0):
        question_restrain = self.question_type_embedding(question_types)[:, 0, :]
        question_categories = self.question_category_embedding(question_categories)[:, 0, :] if self.dataset_name == 'svqa_dataset' else self.motion_feature_fc(question_categories).max(dim=1)[0]
        answers_embedding = self.embedding(encoded_answers)
        answers_lstm, (h_a, c_a) = self.lstm(answers_embedding)
        ...
        for t in range(self.seq_length + 1):
            ...
            if sample_max:
                ...
                # print('it', it.shape)  # (bs, 1)
                # print('logprobs', logprobs.shape)  # (bs, voc_siz)
                sample_logprobs = logprobs.gather(1, it)  # gather the logprobs at sampled positions
                # print('sample_logprobs.shape', sample_logprobs.shape)  # (bs, seq)
