...
class MSVDQADataset(Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        self.question_type = [self.word_to_idx[i["question"].split()[0]] for i in self.json_data]
        ...
        # self.video_ids_to_index = []
        with h5py.File(self.frame_features_path, 'r') as fp:
            self.video_ids = fp['video_id'][()]
        self.video_id_to_index = {str(id): i for i, id in enumerate(self.video_ids)}
        self.motion_feature_path = '../VideoQG/data/MSVD-QA/msvd-qa_motion_feat_24_10clip.h5'
        with h5py.File(self.motion_feature_path, 'r') as f:
            self.video_motion_ids = f['video_id'][()]
        self.video_id_to_motion_index = {str(id): i for i, id in enumerate(self.video_motion_ids)}
    ...
    def _load_video(self, video_id):
        ...
        idx = self.video_id_to_index[str(video_id)]
        vid = self.frame_features['video_id'][idx]
        assert int(vid) == video_id
        feats = self.frame_features["resnet_features"][idx]
        return feats

    def _load_video_motion(self, video_id):
        """loading frame feature"""
        if not hasattr(self, 'motion_features'):
            self.motion_features = h5py.File(self.motion_feature_path, 'r')
        idx = self.video_id_to_motion_index[str(video_id)]
        vid = self.motion_features['video_id'][idx]
        assert int(vid) == video_id
        feats = self.motion_features["resnext_features"][idx]
        return feats
    ...
    def __getitem__(self, index):
        ...
        qc = torch.from_numpy(self._load_video_motion(video_id)).float()
        # qc = a
...
class MSVDQADataLoader(DataLoader):
